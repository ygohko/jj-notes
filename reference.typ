...
- リポジトリディレクトリ内のファイルシステムの状態。それぞれのチェンジがディレクトリとすべての
  ファイルの完全なコピーを保持していることを想像してください。実際には、`jj`はもっと効率的にそれを
  行っています。
...
- 一つかそれ以上の_親_のチェンジ。ですが、親がなく常に空ディレクトリを持つルートのチェンジも
  あります。
- テキストによるチェンジの説明で、コミットメッセージとも呼ばれます。これは常に存在しますが、
  デフォルトは空文字列です。
...
- チェンジへの重複しない文字列のラベル _ブックマーク_があるかもしれません。(`git`と連携する場合、
...
- チェンジにファイルの修正と説明がなく、`@`やブックマークから参照されていない場合、静かに闇へと
  消えていきます。
- チェンジは差分を表現しています。チェンジを移動すると新しい親に差分を適用しようとします。これは
...
(`git pull`がマージコンフリクトを生じることに似ています。)この"ブックマークの衝突"を
どう解決するかはあなた次第です。いくつかの選択肢があります:
